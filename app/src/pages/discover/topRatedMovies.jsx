...
useEffect(() => {
    const fetchData = async () => {
      try {
        // Fetch movie details
        const movieResponse = await axios.get(`https://api.themoviedb.org/3/movie/top_rated/?api_key=${API_KEY}&language=en-US&sort_by=TopRatedity.desc&include_adult=false&include_video=false&page=${index}`);
    ...
    return(
        <div>
          {isSignedIn ? (
            ...
            <h2 className="ms-4 ms-md-5 trailer-link" style={{fontFamily:'Montserrat'}}>Top Rated Movies</h2>
            </div>
